  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        ...
        // Backend wraps as { success, message, data: {...} }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="flex justify-between items-start mb-4">
            ...
            <span
              className={`px-2 py-0.5 text-[10px] font-bold rounded ${
                statusData?.serverStatus === 'Healthy'
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-xs font-bold text-slate-500 tracking-wider">CANDIDATES PROCESSED</h3>
            <span className="px-2 py-0.5 text-[10px] font-bold text-blue-700 bg-blue-100 rounded">
              Live
            </span>
          </div>
          <p className="text-4xl font-bold text-blue-600 mb-1">
            {loading ? '—' : statusData?.candidateCount ?? 0}
          </p>
          <p className="text-sm text-slate-400 font-medium">Total screened CVs</p>
        </div>
      </div>
